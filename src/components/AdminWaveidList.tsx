...
export const AdminWaveidList: React.FC<AdminWaveidListProps> = ({
  ...
  return (
    <section id="proyek" className="py-8 border-b border-neutral-200/70 dark:border-neutral-800/70">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs font-mono font-medium px-2 py-0.5 rounded-full bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400 border border-neutral-200/80 dark:border-neutral-700/80">
            {admins.length}
          </span>
        </div>
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-neutral-900 dark:bg-neutral-100 text-white dark:text-neutral-900 text-xs sm:text-sm font-semibold hover:opacity-90 active:scale-98 transition-all cursor-pointer shrink-0 self-start sm:self-auto shadow-xs"
        >
      ...
      <div className="flex items-center gap-1.5 mb-6 pb-1 overflow-x-auto scrollbar-none">
        {categories.map(cat => (
          <button
            key={cat.id}
        ...
        {filteredAdmins.map(admin => {
          ...
          return (
            <article
              ...
              className="group rounded-xl border border-neutral-200/80 dark:border-neutral-800/80 bg-white dark:bg-neutral-900/60 p-2.5 sm:p-3 flex flex-col hover:border-neutral-300 dark:hover:border-neutral-700 transition-all"
            >
              {/* Foto Admin Proporsional (3:4) */}
              <div className="relative w-full aspect-[3/4] rounded-lg overflow-hidden bg-neutral-100 dark:bg-neutral-800 mb-2.5">
                {!hasImageError ? (
                  <img
                    src={admin.photoUrl}
                    alt={admin.name}
                  ...
                  <div className="w-full h-full flex items-center justify-center font-mono font-bold text-neutral-400 text-lg">
                    {initials}
                  </div>
                )}
                ...
                {admin.subRole && (
                  <span className="absolute top-2 right-2 px-1.5 py-0.5 rounded text-[10px] font-mono font-semibold bg-black/60 text-white backdrop-blur-xs">
                    {admin.subRole}
                  </span>
                )}
              </div>
              ...
              <div className="flex flex-col flex-grow justify-between min-h-[40px]">
